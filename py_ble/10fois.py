    ...
    try:
        ...
        async def process_data():
            while True:
                ...
                for i in range(0, 224, 28):  # 每28字节一组数据，共8组
                    sample = data[i:i + 28]
                    if len(sample) < 28:
                        continue

                    # 时间戳（前4字节），大端 unsigned
                    timestamp_raw = int.from_bytes(sample[0:4], byteorder='big', signed=False)
                    timestamp = logger.start_time + (timestamp_raw / 1000.0)  # 转换为秒

                    led_values = []
                    # 从第4字节开始解析4组 [LED, DARK]
                    for j in range(0, 24, 6):
                        led = int.from_bytes(sample[4 + j: 4 + j + 3], byteorder="big", signed=True) / 8388608.0 * 4.0
                        dark = int.from_bytes(sample[4 + j + 3: 4 + j + 6], byteorder="big",
                                              signed=True) / 8388608.0 * 4.0
